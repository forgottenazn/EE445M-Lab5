...
void ADC_Open(uint32_t channelNum, uint32_t period){
 ...
 // DisableInterrupts();
  ...
  ADC1_ACTSS_R &= ~0x04;    // disable sample sequencer 2
  ADC1_EMUX_R = (ADC0_EMUX_R&0xFFFFF0FF)+0X500; // timer trigger event
  ADC1_SSMUX2_R |= channelNum; // second channel
  ADC1_SSCTL2_R = 0x06;          // set flag and end                       
  ADC1_IM_R |= 0x04;             // enable SS2 interrupts
  ADC1_ACTSS_R |= 0x04;          // enable sample sequencer 2
  ...
  ADC0_ACTSS_R &= ~0x01;    // disable sample sequencer 3
  ADC0_EMUX_R = (ADC0_EMUX_R&0xFFFFFFF0)+0x0005; // timer trigger event
  ADC0_SSMUX0_R = channelNum;
  ADC0_SSCTL0_R = 0x06;          // set flag and end                       
  ADC0_IM_R |= 0x01;             // enable SS3 interrupts
  ADC0_ACTSS_R |= 0x01;          // enable sample sequencer 3
  NVIC_PRI3_R = (NVIC_PRI3_R&0xFF00FFFF)|0x00400000; //priority 2
  NVIC_EN0_R |= 1<<14;              // enable interrupt 17 in NVIC
	
	
  EndCritical(start);
}

void ADC_Init(uint32_t channelNum){ // half a millisecond period
	 volatile uint32_t delay;
  // **** GPIO pin initialization ****
  switch(channelNum){             // 1) activate clock
    case 0:
  ...
  //SYSCTL_RCGCTIMER_R |= 0x01;   // activate timer0 
  delay = SYSCTL_RCGCADC_R;   // allow time to finish activating
	delay = SYSCTL_RCGCADC_R;   // allow time to finish activating
	delay = SYSCTL_RCGCADC_R;   // allow time to finish activating
	delay = SYSCTL_RCGCADC_R;   // allow time to finish activating
  /*
  ...
  TIMER0_TAMR_R = 0x00000002;   // configure for periodic mode, default down-count settings
  TIMER0_TAPR_R = 0;            // prescale value for trigger
  TIMER0_TAILR_R = 40000-1;    // start value for trigger
  TIMER0_IMR_R = 0x00000000;    // disable all interrupts
  TIMER0_CTL_R |= 0x00000001;   // enable timer0A 32-b, periodic, no interrupts
	*/
	/*
  ...
  ADC1_ACTSS_R &= ~0x08;        // 10) disable sample sequencer 2
  ADC1_EMUX_R &= ~0x0000F000;         // 11) seq2 is software trigger
  ADC1_SSMUX3_R = channelNum;         // 12) set channels for SS2
  ADC1_SSCTL3_R = 0x06;         // 13) no TS0 D0 IE0 END0 TS1 D1, yes IE1 END1
  ADC1_IM_R &= ~0x8;           // 14) disable SS2 interrupts
  ADC1_ACTSS_R |= 0x8;         // 15) enable sample sequencer 2
...
//void (*ADCtask2)(unsigned long adc);
int ADC_Collect(uint32_t channelNum, uint32_t fs, void (*task)(unsigned short)){
	ADC_Open(channelNum, 80000000/fs);
	ADCtask = task;
	return 1;
}

volatile unsigned long ADCvalue;
volatile unsigned long ADCvalue2;
//unsigned long ADCarray[102];
int ADCcount = 0;

void ADC0Seq0_Handler(void){
  ADC0_ISC_R = 0x01;          // acknowledge ADC sequence 3 completion
  ADCvalue2 = ADC0_SSFIFO0_R;  // 12-bit result
	
	// keep track of ADC values
	if (ADCcount < 102){
		ADCarray[ADCcount] = ADCvalue2;
		ADCcount++;
	} 
	else {
		ADCcount = 0;
...
uint32_t ADC_In(void){
  ...
  ADCvalue = ADC1_SSFIFO3_R;  		// 3A) read first result
  ADC1_ISC_R = 0x0008;             // 4) acknowledge completion
		
	if (ADCcount < 102){
		ADCarray2[ADCcount] = ADCvalue;
		ADCcount++;
	} 
	else {
		ADCcount = 0;
