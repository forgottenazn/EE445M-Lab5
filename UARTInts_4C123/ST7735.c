...
#define ST7735_LINES_PER_SCREEN ((int) 8)
...
void ST7735_InitR(enum initRFlags option) {
	OS_InitSemaphore(&LCD_Sema4, 0); // wait for initialization
  commonInit(Rcmd1);
  ...
  StTextColor = ST7735_YELLOW;
  ST7735_FillScreen(0);     // set screen to black
	OS_bSignal(&LCD_Sema4);		// Semaphore set to available
}
...
// ************** ST7735_Message ************************
// Prints a text message to the LCD display
// LCD display is serparated logically into a top and botton screen
// Input:  device (0=top, 1=bottom), Line to output to (0-7), text string, numerical number to display next to message
// Output: none
...
void ST7735_Message (int device, int line, char *string, long value){
	...
	count = strlen(string) + strlen(buffer);
	
	OS_bWait(&LCD_Sema4); // wait for LCD to finish
	//determine line to output to
	//lines 0-7 belong to top screen; 8-15 to bottom screen
	outputLine = (device * ST7735_LINES_PER_SCREEN) + line;
	ST7735_SetTextColor(ST7735_WHITE);
	ST7735_SetCursor(0, outputLine);
	...
	OS_bSignal(&LCD_Sema4);	// LCD is finished
	//EndCritical(start);
	return;
}
